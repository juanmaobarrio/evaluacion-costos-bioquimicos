<template>
  <div class="min-h-screen bg-gradient-to-br from-brand-900 via-brand-800 to-brand-600 flex items-center justify-center p-4">
    <div class="w-full max-w-md bg-white border border-brand-100 rounded-3xl p-8 shadow-2xl shadow-brand-950/40 relative overflow-hidden">
      <!-- Glow background decoration -->
      <div class="absolute -top-24 -right-24 w-48 h-48 bg-brand-400/15 rounded-full blur-3xl pointer-events-none"></div>
      <div class="absolute -bottom-24 -left-24 w-48 h-48 bg-brand-300/15 rounded-full blur-3xl pointer-events-none"></div>
      ...
      <!-- Error message -->
      <div v-if="authStore.error" class="mb-5 p-3.5 bg-rose-50 border border-rose-200 rounded-xl flex items-center gap-3 text-rose-700 text-sm">
      ...
      <div class="mt-8 pt-6 border-t border-slate-200 text-center relative z-10">
        <p class="text-xs text-slate-500 mb-3">Acceso rápido con datos semilla:</p>
        <div class="flex gap-2 justify-center">
          <button
            @click="fillCredentials('admin@laboratorio.com', 'admin123')"
            type="button"
            class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 border border-slate-300 rounded-lg text-xs font-medium text-brand-600 transition-colors"
          >
            Admin (Full)
          </button>
          <button
            ...
            type="button"
            class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 border border-slate-300 rounded-lg text-xs font-medium text-brand-400 transition-colors"
          >
            Bioquímico Jefe
          </button>
        </div>
